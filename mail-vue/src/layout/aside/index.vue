<template>
  <el-scrollbar class="scroll">
    <div>
      <div class="title" @click="router.push({name: 'analysis'})" style="cursor: pointer;">
        <Icon icon="mdi:email-outline" width="24" height="24" />
        <div>{{settingStore.settings.title}}</div>
      </div>
      <el-menu :collapse="false" text-color="#fff" active-text-color="#fff" style="margin-top: 10px">
        <el-menu-item @click="openSend" index="new-mail" v-perm="'email:send'">
          <Icon icon="material-symbols:edit-outline-sharp" width="20" height="20" />
          <span class="menu-name" style="margin-left: 21px">{{$t('newMail')}}</span>
        </el-menu-item>
